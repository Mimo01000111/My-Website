...
const accounts = [
    ...
    { id: 48, email: "ruiz_angel@barangay118.gov.ph", password: "user123", role: "user", name: "Angel Rosales RUIZ", status: "active", registered: new Date().toLocaleDateString() }
            ...
            // If no admins in localStorage, add the hardcoded ones
            ...
            // Initialize system logs if empty
        ...
        document.addEventListener('DOMContentLoaded', function() {
            initializeDefaultAccounts(); // Initialize accounts first
            setupEventListeners();
            checkRememberedUser();
            loadLoginContent();
        });
        ...
        // Authenticate user against all accounts
        function authenticateUser(email, password) {
            // Check localStorage first (from Admin Control)
            const storedUsers = JSON.parse(localStorage.getItem('barangayUsers')) || [];
            const storedAdmins = JSON.parse(localStorage.getItem('barangayAdmins')) || [];
            
            // Combine all accounts from localStorage
            const allStoredAccounts = [...storedUsers, ...storedAdmins];
            
            // Check against stored accounts first
            ...
            // If not found in localStorage, check hardcoded accounts as fallback
            const accounts = [
                // Admin accounts
        ...
        function redirectBasedOnRole(role, email) {
            // List of admin emails that should go to PanelAdminView.html
            ...
            switch(role) {
                case "super_admin":
                    ...
                    break;
                case "admin":
                    // Check if the admin is one of the three that should go to PanelAdminView
